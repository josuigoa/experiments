...
class Foldable extends Entity {
	...
	var mesh:Geometry;
	var folding_shadow_shader:Shader;
	...
	public function new(?_options:FoldableOptions) {
		...
		folding_shadow_shader = Luxe.resources.shader("fold_shadow");
		if (folding_shadow_shader != null) {
			folding_shadow_shader.set_float("line_center", .5);
		}
	...
	public function fold(animate:Bool = true, oncomplete:Foldable->Void = null) {
		...
		if (animate) {
			if (shader != null){
				mesh.shader = folding_shadow_shader;
			}
	...
	function updateFolding() {
		...
		if (folding_shadow_shader != null) {
			folding_shadow_shader.set_float("line_alpha", 1.-folding_value);
			folding_shadow_shader.set_int("is_horizontal", (direction == UP || direction == DOWN) ? 1 : 0);
		}
